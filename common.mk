...
pr: ## Prepare for opening a PR: bump to the next pre-release version and commit
	...
	read ans; \
	[ "$$ans" = y ] || [ "$$ans" = Y ] || { echo aborted; exit 1; }; \
	cd '$(ROOT)' && uv --directory '$(PKG)' version "$$version" && uv --directory '$(PKG)' sync --frozen --all-groups --all-extras && git add '$(PKG)/pyproject.toml' '$(PKG)/uv.lock' && git commit -m "bump '$(PKG)' to $$version"; \
	repo_name="$$(git config --get remote.origin.url | sed -E 's/.*github.com[:\/](.*)\.git/\1/')"; \
